...
export default class DummyRadarDataProvider {
    ...
    addItem (radius, angle) {
        return new Promise(function (resolve, reject) {
            var label = prompt('Please enter the label of the new blip:\n(NOTE: If this is the 2nd time you are seeing this message click "ok" without typing anything)', '');
            if (label === null || label === '') {
                reject();
            } else {
                var link = prompt('Copy and Paste the JIRA link of project here');
                this.data.push({'id': this.data.length + 1, 'radius': radius, 'angle': angle, 'label': label, 'link': link, 'movementId': 1});
                resolve(this.data[this.data.length - 1]);

                this.sendUpdate(); // do not need this. infront of value because the function is using (.bind(this))
            }
        }.bind(this));
    }
